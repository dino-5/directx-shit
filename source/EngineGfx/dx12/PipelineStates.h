...
struct DepthState
{
    ...
    DepthState(bool Enabled,
               DepthWriteMask Mask,
               ComparisonFunc Func)
    : isEnabled(Enabled), writeMask(Mask), depthFunc(Func)
    {}
    DepthState()=default;
};
...
struct StencilOpDesc
{
    ...
    StencilOpDesc(StencilOP fail= StencilOP::KEEP,
...
struct StencilState
{
    ...
    StencilState(bool Enabled = true,
                 u8 read = 0xff, u8 write = 0xff,
                 StencilOpDesc front = StencilOpDesc(),
                 StencilOpDesc back = StencilOpDesc()) 
        : isEnabled(Enabled), readMask(read), writeMask(write),
...
{
public:
    RasterizerState(CullMode cull = CullMode::BACK,
